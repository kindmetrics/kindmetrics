...
describe Api::Domains::Data::EntryPages do
  before_each do
  ...
  it "see empty pages list for domain" do
    ...
    response = AppClient.auth(token).exec(Api::Domains::Data::EntryPages.with(domain.id))
  ...
  it "see 1 in pages list for domain" do
    ...
    response = AppClient.auth(token).exec(Api::Domains::Data::EntryPages.with(domain.id))
    response.status_code.should eq(200)
    response.body.should contain("test/rrr")
  end
end
