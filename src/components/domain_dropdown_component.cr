        ...
        div class: "absolute right-0 mt-2 w-full hidden z-10", data_target: "dropdown.menu" do
          div class: "bg-white shadow-lg rounded overflow-hidden border" do
            (@domains || [] of Domain).each do |d|
              link d.address, to: Domains::Show.with(d), class: "hover:no-underline block px-5 py-4 text-gray-900 bg-white hover:bg-menu-button whitespace-no-wrap"
            end
            link "Add Domain", to: Domains::New, class: "hover:no-underline block px-5 py-4 text-gray-900 bg-white hover:bg-menu-button whitespace-no-wrap"
          end
        end
      end
    end
