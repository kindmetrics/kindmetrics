    ...
    tr class: index.odd? ? "bg-gray-200" : "bg-white" do
      td class: "w-4/6 py-2" do
        if !event.referrer_domain.nil? && event.referrer_domain.not_nil! == current_domain.address
          ...
          a class: "block px-2 hover:underline truncate", href: current_user.nil? ? Share::Referrer::Show.with(current_domain.hashid, event.referrer_source.to_s, period.to_s).url : Domains::Referrer::Show.with(current_domain.hashid, event.referrer_source.to_s, period.to_s).url do
