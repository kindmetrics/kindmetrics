class Events::Create < ApiAction
  ...
  post "/api/track" do
    ...
    SaveEvent.create(name: data["name"].to_s, user_agent: user_agent, referrer: referrer.to_s, referrer_domain: referrer.host, url: url.to_s, path: url.path, source: data["source"].to_s, screen_width: data["screen_width"].to_s, operative_system: browser.try { |b| b.os_name }, domain_id: domain.id, user_id: user_id) do |operation, event|
      if event
        Log.debug { "Yay, saved!" }
      else
