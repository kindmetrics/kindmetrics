class PasswordResets::NewPage < AuthLayout
  ...
  private def render_password_reset_form(op)
    form_for PasswordResets::Create.with(@user_id) do
      ...
      m Shared::Field.new(op.password_confirmation, "Confirm Password"), &.password_input(append_class: "w-full bg-white text-gray-900 focus:bg-white border border-gray-400 hover:border hover:border-blue-500 focus:text-black rounded p-2 my-2 leading-tight transistor")
