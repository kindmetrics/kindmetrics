class Domains::NewPage < AdminLayout
  ...
  def render_domain_form(op)
    form_for Domains::Create do
      ...
      m Shared::Field.new(operation.address, "Host (domain.com)"), &.text_input(autofocus: true, append_class: "w-full bg-gray-300 text-gray-900 focus:bg-white focus:text-black rounded p-2 my-2 text-sm leading-tight")
