class Domains::Data::SourcePage < Domains::Data::BasePage
  ...
  def render_table
    m DashboardTableComponent, first_header: "Source", second_header: "Visitors" do
      @referrers.each do |r|
        render_row(r)
      end
    end
  ...
  def get_url(row)
    if current_user.nil?
      Share::Show.with(share_id: domain.hashid, goal_id: goal.try { |g| g.id } || 0_i64, site_path: site_path, source_name: row.referrer_source.to_s, period: period).url
    else
      Domains::Show.with(domain_id: domain.id, goal_id: goal.try { |g| g.id } || 0_i64, site_path: site_path, source_name: row.referrer_source.to_s, period: period).url
    end
  end
end
